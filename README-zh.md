...
4. 打开你的 Laravel 项目的 `.env` 文件，然后设置 `mysql` 的 `DB_HOST` 和 `redis` 的`REDIS_HOST`。
...
- **数据库引擎:** - MySQL - PostgreSQL - MariaDB - MongoDB - Neo4j
- **缓存引擎:** - Redis - Memcached
- **PHP 服务器:** - NGINX - Apache2 - Caddy
- **PHP 编译工具:** - PHP-FPM - HHVM
- **消息队列系统:** - Beanstalkd (+ Beanstalkd Console)
- **工具:** - Workspace (PHP7-CLI, Composer, Git, Node, Gulp, SQLite, Vim, Nano, cURL...)
...
1 - 运行容器: _(在运行 `docker-compose` 命令之前，确认你在 `laradock` 目录中_
...
2 - 打开你的 Laravel 的 `.env` 文件 然后 配置 `redis` 的 `REDIS_HOST`
...
PHP-FPM 拓展务必安装在 `php-fpm/Dockerfile-XX`. _(用你 PHP 版本号替换 XX)_.
...
> 更多关于 PHP 基础镜像, 请访问 [PHP Docker 官方镜像](https://hub.docker.com/_/php/).
...
从 Docker Toolbox (VirtualBox) 移动到 Docker Native (for Mac/Windows)，需要从 Laradock v3._ 升级到 v4._:
